...
module.exports = class Hyperlinc {
  ...
  async getProfiles(ids){
    ...
    return await Promise.all(
      ids.map(id => this.getIdentity(id).getProfile())
    )
    // return ids.map((_, i) => {
    //   return {
    //     realname: `fake${i}`,
...
class Identity {
  ...
  async append(...updates){
    await this.core.append(...updates.map(serialize))
  }
  ...
  async getProfile(){
    console.log('waiting for', this.core)
    await this.core.update()
    console.log('updated', this.core)
    return {}
  }
}
